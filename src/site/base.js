...
self.sauceBaseInit = function sauceBaseInit(extId, extUrl, name, version) {
    ...
    class DBStore {
        ...
        async *values(query, options={}) {
            //console.error("Values iter considered dangerous on safari", query, options);
            let iter;
            let i = 0;
        ...
        async *_cursorOrig(query, options={}) {
            if (!this._started) {
                await this._start();
            }
            const req = this._cursorRequest(query, options);
        ...
        async *_cursor(query, options={}) {
            ...
            const results = [];
            try {
                //testit.then(console.warn).catch(console.error);
                while (count < limit) {
                    const cursor = await new Promise((resolve, reject) => {
                        req.onsuccess = ev => resolve(req.result);
                        req.onerror = ev => resolve(req.error);
                    });
                    if (!cursor) {
                        break;
                        return;
                ...
                console.error({count, query, options, e, results});
                throw e;
            }
        }
